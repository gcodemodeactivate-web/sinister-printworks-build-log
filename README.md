...
[![Phases](https://img.shields.io/badge/PHASES-44_COMPLETE-FF4500?style=for-the-badge&labelColor=0D0D0D)](#build-log)
...
[![Deploys](https://img.shields.io/badge/DEPLOYS-14_GREEN-FF4500?style=for-the-badge&labelColor=0D0D0D)]()
...
### Phase 44: Education-First Website Overhaul — 10 Branches, 4 Phases, 52 Commits
![Status](https://img.shields.io/badge/STATUS-COMPLETE-FF4500?style=for-the-badge&labelColor=0D0D0D) ![Date](https://img.shields.io/badge/DATE-April_11_2026-8B0000?style=for-the-badge&labelColor=0D0D0D)

The largest single overhaul in the project's history. A customer-experience audit identified eight structural problems with the live site: jargon without explanation, AI-writing tells (283 em-dashes across the codebase), misleading FDM framing, leaked competitive intel (the "Our Hardware" section), friction in the quote flow, a weak About page, pay-to-win gamification, and zero knowledge base. All eight shipped as a single coordinated overhaul across four phases.

**Execution model.** Three-role pipeline per branch: an Expert that writes the code, a Devil's Advocate that self-critiques the diff with severity labels, and a QA Gatekeeper that can kick back to the Expert up to three times before approval. Every branch ran in its own git worktree with a dedicated dev-server port to avoid stop-hook collisions across parallel agents. Phases gated each other: Phase A had to ship the knowledge base + TechTerm primitive before Phase B's six parallel page overhauls could consume it.

**Phase A — Foundations (blocks Phase B).**
- **A1: Global nav/footer/copy hygiene.** 5-item top nav (`Home · Start Your Project · Knowledge Base · About · More`), clickable grandiose SINISTER PRINTWORKS logo routing to `/`, responsive `clamp()` sizing, Footer contact cards with lucide icons instead of emojis, 283 em-dashes eradicated across 112 files via a new `scripts/strip-ai-tells.mjs` codemod, 9 emoji-bearing files swept. "Junction City Premium Manufacturing Hub" badge wired to the new `/location` page.
- **A2: Knowledge base + TechTerm primitive.** New `/knowledge-base` long-scroll page with 21 anchor-linked entries covering printing technologies (FDM, Resin), 8 FDM materials, 4 resin materials, 3 file formats (STL, 3MF, CAD), 3 physical properties (UV, carbon fiber, tolerance), and Ean's warrant-officer story with a verbatim Army Warrant Officer definition. The `<TechTerm>` component is a Radix HoverCard + `next/link` primitive that renders SSR-safe markup, degrades to tap-navigate on touch, handles keyboard focus, and deep-links to `/knowledge-base#<id>` with a flash-highlight on arrival. `content/knowledge-base.ts` is the typed single source of truth feeding both the hover-card summaries and the full page body.

**Phase B — Six parallel branches (depend on A).**
- **B1: Homepage ScrollStory narrative overhaul.** FDM and resin reframed as co-equal offerings (not "resin is better"). Every jargon term wrapped in `<TechTerm>`. A "part they stopped making" hover card that references a real discontinued consumer part. "YOU spent several nights perfecting the CAD files" rewrite of the inventor act. Hero trust bar unified to "2 to 4 business days". PrinterShowcase (the "Our Hardware" section exposing Anycubic + Bambu Lab machines to competitors) archived entirely — the scroll story's "start your project" CTA is now the last content block before the Footer.
- **B2: Upload/quote flow overhaul.** Legal agreement relocated from a full-page gate to an inline scroll-bottom block at the bottom of the form — the customer can interact with the dropzone, material selector, and form fields BEFORE ever seeing the agreement. `CONSENT_VERSION` preserved so previously-consented users skip the re-prompt. New `MaterialSelector` component with an FDM/Resin technology toggle, 8 FDM materials and 4 (now 5) resin materials, TechTerm hovers on every option, a color picker that hides on resin. Quote API accepts `printProcess` on the existing nullable `Quote.printProcess` column (zero schema change). Inline dual-pipeline density lookup + `/api/quote/analyze` + `/api/quote/estimate` density and cost tables taught PET-CF, ABS-CF, and every resin variant so weight estimates stop silently falling through to PLA.
- **B3: Materials catalog (new `/materials` route).** New scroll-driven datasheet catalog with a persistent left-rail taxonomy sidebar and one big right-side panel per material. 12 materials (all 8 FDM + 4 resin) each rendered as category pill + kind tag + TechTerm headline + 1-sentence hook + 3 spec bullets + abstract SVG visual + "Open full entry" deep link into the knowledge base. Distinct from the About page's story-arc MaterialsScrollStory.
- **B4: About page overhaul.** Eliminated the black void before the eagle animation by rendering the crimson radial gradient from the first paint. Enlarged the eagle to `clamp(300px, 60vh, 800px)`. Widened text overlays to parity with the homepage scroll story. Rewrote Act 1 to remove "zero-compromise" phrasing, replaced with "every polymer chosen for its fitness to the application". Added a "Warrant Officer" hover-card that deep-links to the verbatim Army WO definition. Replaced "years of military service" with "over a decade of service".
- **B5: Gamification redesign.** Deleted loot chests entirely — 14 files removed including ChestShop, ChestOpeningAnimation, XPBoostIndicator, `lib/gamification/chests.ts`, 3 `/api/gamification/chest/*` routes, 2 `/api/gamification/boosts/*` routes, the `/dashboard/shop` route, and the admin Chest Stats tab. No migration logic needed (zero existing customers). Tier 8/9/10 rebalanced from 1300/1600/2000 points to **2500/4500/8000** — the 15% discount at tier 10 now takes genuine loyalty to unlock. Character showcase redesigned to 2-character-per-rarity full-viewport scenes so each tier feels epic. "Save more" → "save more PERMANENTLY". New 30-day `LoyaltyCalendar` model: one ≥$15 purchase in a sliding 30-day window earns one cosmetic from the user's current rarity pool, with benefit escalation `none` → `better` → `best` across months 1, 2, and 3. 5 rarity pools × 3 cosmetics = 15 items. Prisma migration `20260411_drop_chests_add_loyalty` drops the chest tables and creates `loyalty_rewards`.
- **B6: Location page (new `/location` route).** Scroll-driven pan-out animation: Junction City → Kansas → continental US. 74 shipping-destination pins across all 50 states plus DC. The shipping provider name never appears in user-facing copy (enforced by grep in the Devil's Advocate pass). Respects `prefers-reduced-motion` with a static final-frame fallback.

**Phase C — Integration sweep.**
- **C1.** Cross-branch em-dash/emoji re-sweep caught one "Zero compromises" residue in ScrollStory and two emojis in `app/upload/page.tsx` — all fixed. TechTerm anchor validation confirmed every `term=` string in the codebase resolves against `content/knowledge-base.ts`. All 8 target routes curl-verified at 200. Turbopack production build green. Header mobile nav toggle gained `aria-label`, `aria-expanded`, and `aria-controls`.

**Phase D — Polish before deploy.** Ean's 6-item wake-up list before merging.
- **D1: Discontinued part research.** Replaced the placeholder Atari 2600 cartridge shell with the **Nokia 3310 rear battery cover**. Rationale documented in `docs/overhaul/D1_DISCONTINUED_PART_DECISION.md`: Nokia stopped making the original 3310 around 2005, the phone is the archetype of "the unbreakable Nokia" in pop culture, the battery cover is structural (not an accessory), it's plain thermoplastic printable in PETG or ABS, and 25-year-old Nokia hardware is legally safe under nominative fair use with an attribution footer. Rewrote the `DiscontinuedPartCard` copy, the Act 2 / Act 3 body text, the `PartScene` + `PrototypeScene` SVG geometry (same battery-cover silhouette reused in 4 instances for visual continuity), and the standalone placeholder SVG asset.
- **D2: All C1 minor-debt items cleaned.** `prefers-reduced-motion` honored in both the homepage `ScrollStory` and the About `MaterialsScrollStory` with RAF-loop disable + static fallback. `/upload` route now mounts the site `<Header>` and `<Footer>` via an `app/upload/layout.tsx` wrapper (a pre-existing gap from main). Hero trust bar unified to "2 to 4 business days" to match the scroll story. Dead-code cleanup: deleted `components/ModelViewerEngine.tsx`, `components/ui/ModelCard.tsx`, `components/ui/PrintEstimate.tsx`, and `app/preview/[filename]/` (all verified unused via grep first). Em-dashes stripped from `prisma/seed.ts` and `public/sw.js`.
- **D3: Amazon-grounded resin pricing + custom material request.** Replaced the pre-launch $0.18/ml proposed rate with a new baseline rooted in 2026-04-11 live Amazon retail data (Sunlu Water-Washable at $15.99/kg, Anycubic Tough 2.0 at $23.79/kg, Anycubic Standard at $13.99/kg, Anycubic High-Clear at $25.49/kg, Anycubic ABS-Like 3.0 at $15.99/kg). At 1.10 g/ml density and an 8× markup covering wash/cure labor, FEP wear, IPA consumables, and small-batch margin: Standard $0.12/ml, Water-Washable $0.12, ABS-Like $0.14, Tough $0.16, High-Clear $0.18. Added a fifth resin option (**High-Clear**) to the MaterialSelector. Added a "**Request a custom material**" card with an expandable textarea for free-text filament requests — when picked, the material column is replaced with a `Custom (see notes)` sentinel and the textarea content is merged into `engineeringMaterialNotes` with a `CUSTOM MATERIAL REQUEST` prefix so the admin flags the quote for manual sourcing and pricing.

**Post-merge hotfix.** Production curl-verification after the main merge caught two leaked loot-chest references that slipped past B5's sweep: the `/showcase` metadata description (in `app/showcase/layout.tsx`) still said "open loot chests for rare items", and `components/ui/CharacterAvatar.tsx` still had a `Forge Shop` nav link pointing at the deleted `/dashboard/shop` route. Both fixed in commit `bfcd122` and deployed immediately.

**Deploy ledger:** 2 production deploys (initial overhaul merge + the hotfix), both built via Turbopack in ~55 seconds each, both aliased to `sinisterprintworks.net` with zero rollbacks. The prisma migration ran automatically during the Vercel build via `prisma migrate deploy` in the `npm run build` script.

**Scale:** 52 commits across 13 branches (A1, A2, B1, B2, B3, B4, B5, B6, C1, D1, D2, D3, plus 3 merge branches), 10 new files in `app/`, 6 new components, 14 deleted files, 1 new prisma model, 1 applied prisma migration, 0 new tsc errors (75 pre-existing baseline unchanged), 0 new lint errors. Every new page (`/knowledge-base`, `/materials`, `/location`) respects `prefers-reduced-motion`, has exactly one `<h1>`, zero em-dashes in user-facing strings, and a SSR-safe deep-link primitive that never leaves the user stranded at a 404.
